...
//  image.src = './resource/curved-road.png';
  ...
  Element button = query('#blur');
  button.onClick.listen(doBlur);
  
  button = query('#fadeOut');
  button.onClick.listen(fadeOut);
 }

  /**
   * Fade out using a class.
   * WILL NOT WORK DUE TO CORS
   */
  void fadeOut(MouseEvent e) {
    String clsName = 'faded';
    Set<String> clsses = canvas.classes;

    if (clsses.contains(clsName)) {
      clsses.remove(clsName);
      clsses.add('fadein');
    } else {
      clsses.remove('fadein');
      clsses.add(clsName);
    }
  }
  
  void doBlur(MouseEvent e) {
    Set<String> clsses = canvas.classes;

    if (clsses.contains('blur')) {
      clsses.remove('blur');
    } else {
      clsses.add('blur');
    }
