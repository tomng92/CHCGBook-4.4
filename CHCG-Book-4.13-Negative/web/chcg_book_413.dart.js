...
$.main = function() {
  ...
  $.image.set$src("./resource/curved-road.png");
  $.image.get$onLoad().listen$1(new $.main_anon());
  document.query$1("#negativeButton").get$onClick().listen$1($.makeNegative);
};

$.makeNegative = function(e) {
  var imagedata, data, t1, i, t2, t3, t4, t5;
  $.Primitives_printString("bouton clicked by thanh");
  imagedata = $.ctx.getImageData$4(0, 0, $.canvas.get$width(), $.canvas.get$height());
  data = imagedata.get$data();
  if (typeof data !== "object" || data === null || (data.constructor !== Array || !!data.immutable$list) && !data.$isJavaScriptIndexingBehavior())
    return $.makeNegative$bailout(1, imagedata, data);
  for (t1 = data.length, i = 0; i < t1 - 4; i += 4) {
    t2 = data[i];
    if (typeof t2 !== "number")
      throw $.iae(t2);
    data[i] = 255 - t2;
  ...
  $.ctx.putImageData$3(imagedata, 0, 0);
};

$.makeNegative$bailout = function(state0, imagedata, data) {
  var t1, i, t2, t3, t4, t5;
  for (t1 = $.getInterceptor$JSArrayJSString(data), i = 0; $.CONSTANT.$lt(i, $.$$sub(t1.get$length(data), 4)); i += 4) {
    t2 = t1.$index(data, i);
    if (typeof t2 !== "number")
      throw $.iae(t2);
    t1.$indexSet(data, i, 255 - t2);
    t3 = i + 1;
    t4 = t1.$index(data, t3);
    if (typeof t4 !== "number")
      throw $.iae(t4);
    t1.$indexSet(data, t3, 255 - t4);
    t3 = i + 2;
    t5 = t1.$index(data, t3);
    if (typeof t5 !== "number")
      throw $.iae(t5);
    t1.$indexSet(data, t3, 255 - t5);
  }
  $.ctx.putImageData$3(imagedata, 0, 0);
};
...
$.makeNegative.call$1 = $.makeNegative;
$.makeNegative.$name = "makeNegative";
$.run.call$1 = $.run;
$.run.$name = "run";
...
//@ sourceMappingURL=chcg_book_413.dart.js.map
